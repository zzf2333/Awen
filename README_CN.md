...
| **文件补全** | Warp 风格的目录/文件路径补全，支持 fuzzy 匹配 | <10ms |
...
**Full（默认）** — 下拉菜单带来源标签、风险面板、失败修复面板。Up/Down 导航、Enter 执行、Tab 接受到命令行编辑。文件路径补全仅作为 ghost text 显示，不进入下拉菜单。
...
└── layer/
    ...
    ├── risk.rs          # 输入 → 危险警告
    ├── filesystem.rs    # 目录/文件路径补全（仅 ghost text）
    └── history_import.rs
plugin/
